...
function makeKurz(q: string): string {
  let t = String(q ?? "");
  t = t.replace(/\s+/g, " ").trim();

  // kleine Normalisierung für Kurzform (optional, aber sauber)
  // "Ich suche ..." -> "such ..."
  if (/^ich\s+suche\b/i.test(t)) {
    t = t.replace(/^ich\s+suche\b/i, "such");
  }
...
function makeUmgang(q: string) {
  ...
  if (/^ich\s+suche\b/i.test(t)) return t.replace(/^ich\s+suche\b/i, "such");
  return "Hast du " + t;
}
