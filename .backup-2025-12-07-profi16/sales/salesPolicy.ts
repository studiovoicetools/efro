...
export function applySalesPolicy(input: SalesPolicyInput): SalesPolicyOutput {
  // SCHRITT 4 FIX: unknownTerms aus input holen, falls verfügbar
  const unknownTerms = (input as any).unknownTerms || [];
  ...
  const salesDecision = computeSalesDecision({
    ...
    unknownTerms: unknownTerms, // SCHRITT 4 FIX: unknownTerms übergeben
