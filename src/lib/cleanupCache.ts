import { createClient } from "@supabase/supabase-js";

const supabaseUrl =
  process.env.SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL!;
const supabaseKey =
  process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY!;
const supabase = createClient(supabaseUrl, supabaseKey);

/**
 * Löscht alte, ungenutzte Cache-Einträge aus Supabase.
 *  - Nur audio-cache
 *  - älter als 30 Tage
 *  - nicht als "protected" markiert
 */
export async function cleanupCache() {
  console.log("🧹 Starte Cache-Bereinigung…");

  const { data, error } = await supabase
    .from("cache_audio")
    .select("id, audio_url, created_at, last_used, protected")
    .lte("created_at", new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString())
    .eq("protected", false);

  if (error) {
    console.error("❌ Fehler beim Abrufen alter Caches:", error);
    return;
  }

  if (!data?.length) {
    console.log("✅ Kein alter Cache zum Löschen gefunden.");
    return;
  }

  for (const row of data) {
    try {
      // Datei im Storage löschen
      const path = row.audio_url.split("/storage/v1/object/public/")[1];
      await supabase.storage.from("public").remove([path]);
      await supabase.from("cache_audio").delete().eq("id", row.id);
      console.log(`🗑️ Gelöscht: ${row.audio_url}`);
    } catch (err) {
      console.error("⚠️ Fehler beim Löschen:", err);
    }
  }

  console.log("✅ Cache-Bereinigung abgeschlossen!");
}
// Automatische Ausführung, wenn Render-Cron oder Server startet
if (process.env.AUTO_CLEANUP === "true") {
  cleanupCache().then(() => {
    console.log("🧹 Cache-Cleanup automatisch ausgeführt.");
  });
}
