...
import { extractNumbersForBudget, extractUserPriceRange, UserPriceRange } from "./budget";
...
function filterProducts(
  ...
  const budgetResult = extractUserPriceRange(text);
  let userMinPrice = budgetResult.minPrice ?? null;
  let userMaxPrice = budgetResult.maxPrice ?? null;
  ...
  const explicitUnder =
    /\b(unter|bis|höchstens|hochstens|hoechstens|max(?:imal)?)\b/.test(textLowerForBudget);
   ...
   *
   * Hier interpretieren wir die vom Budget-Parser erkannten Werte (userMinPrice/userMaxPrice)
   * und passen sie anhand der Sprache an, z. B.:
  ...
  // 3a) Sprachbasierte Korrektur für "über / unter / bis / maximal"
  // Wir nutzen hier den bereits normalisierten Text "cleaned".
  const lowered = cleaned.toLowerCase();

  const hasOverToken = /\b(über|ueber|uber|ab|mindestens|minimum|mehr als|größer als|groesser als)\b/.test(
    lowered
  );
  const hasUnderToken =
    /\b(unter|bis|höchstens|maximal|weniger als|nicht mehr als)\b/.test(
      lowered
    );

  if (userMinPrice !== undefined || userMaxPrice !== undefined) {
    let rawMin = userMinPrice ?? null;
    let rawMax = userMaxPrice ?? null;

    // Beispiel: "über 500 Euro" → Parser gibt max=500, min=undefined
    // Wir interpretieren das als: min >= 500, kein oberes Limit.
    if (hasOverToken && rawMax !== null && rawMin === null) {
      rawMin = rawMax;
      rawMax = null;
    }

    // (nur zur Vollständigkeit – aktuell selten genutzt)
    // Falls irgendwann ein Pattern "mindestens 50 Euro" → min=50 ohne max liefern würde.
  ...
  // Zweite Korrektur-Schicht direkt auf minPrice/maxPrice,
  // falls der Budget-Parser "über/unter" noch nicht korrekt abgebildet hat.
  const textLowerForPrice = text.toLowerCase();
  const hasOverToken =
    /\b(über|ueber|uber)\b/.test(textLowerForPrice);
  const hasUnderToken =
    /\b(unter|bis|höchstens|hochstens|hoechstens|max(?:imal)?)\b/.test(
      textLowerForPrice
    );

  if (hasOverToken && !hasUnderToken && minPrice === null && maxPrice !== null) {
    // "über 100 €" → minPrice = 100, keine Obergrenze
    minPrice = maxPrice;
    maxPrice = null;
  }

  if (hasUnderToken && !hasOverToken && maxPrice === null && minPrice !== null) {
    // "unter 50 €" → maxPrice = 50, keine Untergrenze
    maxPrice = minPrice;
    minPrice = null;
  }
  ...
  if (minPrice !== null || maxPrice !== null) {
    ...
    // Zusätzlich: sehr niedrige globale Budgets (z. B. "unter 20 €" ohne Kategorie)
    ...
    if (
      ...
      console.log("[EFRO SB] PRICE RANGE NO MATCH", {
        ...
        priceRangeInfo,
        note: "Keine oder nur sehr unpassende Produkte im gewünschten Preisbereich gefunden.",
      });
    }
