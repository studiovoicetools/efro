...
function filterProducts(
   ...
   *
   * Interpretiert die vom Budget-Parser erkannten Werte (userMinPrice/userMaxPrice)
   * und passt sie anhand der Sprache an, z. B.:
  ...
  // 3a) Ausgangspunkt: vom Parser erkannte Werte
  let rawMin: number | null = userMinPrice ?? null;
  let rawMax: number | null = userMaxPrice ?? null;

  // 3b) Sprachbasierte Korrektur für "über / unter / bis / maximal"
  const loweredForBudget = cleaned.toLowerCase();

  const hasOverToken = /\b(über|ueber|uber|ab|mindestens|minimum|mehr als|größer als|groesser als)\b/.test(
    loweredForBudget
  );
  const hasUnderToken =
    /\b(unter|bis|höchstens|hochstens|hoechstens|maximal|weniger als|nicht mehr als)\b/.test(
      loweredForBudget
    );

  if (rawMin !== null || rawMax !== null) {
    // Beispiel: "über 500 Euro" → Parser gibt max=500, min=undefined
    // → min >= 500, kein oberes Limit
    if (hasOverToken && rawMax !== null && rawMin === null) {
      rawMin = rawMax;
      rawMax = null;
    }

    // Beispiel: "unter 50 Euro" → Parser liefert nur min=50
    // → max <= 50, keine Untergrenze
    ...
    // Keine explizite Preisangabe → Premium/Bargain steuern nur Sortierung
    minPrice = null;
    maxPrice = null;
  }
