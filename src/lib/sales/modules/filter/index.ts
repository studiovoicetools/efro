...
// Alias: in dieser Funktion ist "cleaned" einfach der Text aus runSellerBrain
  ...
  if (computedPriceRangeNoMatch !== undefined) {
    priceRangeNoMatch = computedPriceRangeNoMatch;
  }
  ...
  if (minPrice !== null || maxPrice !== null) {
    ...
    // EFRO K2/K4/K8/K13 Fix: priceRangeNoMatch = true, wenn keine Produkte IM Budget sind
  ...
  // EFRO D8 Fix: Finale Budget-Prüfung – entferne Produkte über explizitem Max-Budget,
  // wenn priceRangeNoMatch === true (keine Produkte IM Budget gefunden)
  // EFRO K2/K4/K13 Fix: Überspringe diese Prüfung bei unrealistischem Budget,
  // da wir bereits die günstigsten Produkte aus aboveBudget genommen haben
  const hasExplicitMaxBudget =
    userMaxPrice !== null && userMaxPrice !== undefined;

  if (hasExplicitMaxBudget && priceRangeNoMatch && !isUnrealisticallyLow && !isUnrealisticallyHigh && userMaxPrice !== null) {
    const beforeFinalFilter = finalProducts.length;

    finalProducts = finalProducts.filter((p) => {
      const price =
        typeof p.price === "number" ? p.price : Number(p.price);
      return !Number.isNaN(price) && userMaxPrice !== null && price <= userMaxPrice;
    });

    if (beforeFinalFilter !== finalProducts.length) {
      console.log("[EFRO Budget Fallback Filter]", {
        text: text.substring(0, 80),
        userMaxPrice,
        beforeFinalFilter,
