 ...
 if (effectiveCategorySlug) {
    ...
    if (filteredByCategory.length > 0) {
      ...
      if (beforeCategoryFilterCount > 0) {
        ...
        // HARDCORE SAFETY: category filter must not zero-out all candidates
        // If it yields 0 matches, keep the pre-filter candidates and continue with scoring/ranking.
  ...
  if (candidates.length === 1) {
    ...
    console.log("[EFRO SB] FINAL products", {
      ...
      products: finalProducts.map((p) => ({
        title: String(p.title ?? "").substring(0, 50),
        price: p.price ?? null,
...
function isAmbiguousBoardQuery(text: string): boolean {
  ...
  if (hasExactSmartphoneModelName && recommended.length > 0 && effectiveCategorySlug === "elektronik") {
    ...
    if (!modelNameInRecommended) {
      ...
      if (effectiveCategorySlug === "elektronik") {
        // recommended kann const sein -> Array leeren statt reassign
 ...
 let missingCategoryHint: string | null = null;
  ...
  if (categoryHintsFromText.length > 0) {
    ...
    if (!existsInCatalog) {
      ...
      const effectivePriceRangeNoMatch = Boolean(priceRangeNoMatch) || Boolean(missingCategoryHint);
    }
  }
  ...
  if (priceRangeNoMatch) {
    ...
    finalReplyText = buildReplyText(
      ...
      recommended,
      aiTrigger,
  (Boolean(priceRangeNoMatch) || Boolean(missingCategoryHint)),
      priceRangeInfo,
 ...
 const signalText =
  ...
  if (salesPolicyOutput.primaryAction === "ASK_CLARIFICATION" &&
    ...
    // PROFI: NO_PRODUCTS_FOUND gilt nur, wenn wir WIRKLICH 0 Treffer haben.
    const __recCount = (recommended?.length ?? 0);

    if (__recCount > 0) {
      // Treffer vorhanden -> Produkte behalten, NO_PRODUCTS_FOUND entfernen.
      (salesPolicyOutput as any).notes = (salesPolicyOutput.notes ?? []).filter((n) => n !== "NO_PRODUCTS_FOUND");
      console.log("[EFRO SB ASK_CLARIFICATION] NO_PRODUCTS_FOUND ignored (products exist)", {
        count: __recCount,
      });
    } else {
      // Originalverhalten: wirklich 0 -> leer lassen
      console.log("[EFRO SB ASK_CLARIFICATION] Produkte entfernt (NO_PRODUCTS_FOUND)", {
        originalCount: __recCount,
        finalCount: 0,
        salesNotes: ["NO_PRODUCTS_FOUND"],
      });
      recommended = [];
    }
  }
