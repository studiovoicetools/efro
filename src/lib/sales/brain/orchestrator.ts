...
import {
  SellerBrainAiTrigger,
  decideAiTrigger,
} from "@/lib/sales/modules/aiTrigger";
 ...
 */
export const staticProductHints: ProductHint[] = productHints.map(
  (keyword) => ({
    keyword,
    weight: 1,
  }),
...
function detectExplanationMode(text: string): ExplanationMode | null {
  ...
  const bargainExclusionWords = [
    ...
    "preiswerteste",
    "preiswertestes",
    "günstigstes snowboard",
    "billigstes snowboard",
  ];
  ...
  const hasExplainPattern =
    /erkl(?:ä|ae|a)r(?:e|st|en)?\s+(?:mir\s+)?(?:bitte\s+)?(?:mal\s+)?(?:genau\s+)?(?:wie|dass|ob)/i.test(
      normalized,
    ) ||
  ...
  const hasErklarMirPattern =
    /^erkl(?:ä|ae|a)r(?:e|en)?\s+mir\s+(?:bitte\s+)?/i.test(normalized);
  ...
  if (
    ...
    // Anwendung / benutzen
    if (
      EXPLANATION_MODE_KEYWORDS.usage.some((w) => t.includes(w)) ||
      hasUsagePattern
    ) {
      return "usage";
    }
...
function patchProductRelatedForWax(
  ...
  if (
    !result.isProductRelated &&
    explanationMode &&
    /\b(wax|wachs)\b/i.test(normalizedText)
  ) {
...
function handleWaxExplanation(
  ...
  console.log(
    "[EFRO Explanation Wax] Wax-Produkte gefunden (keine Kategorie erkannt)",
    {
      text,
...
function isProductRelated(text: string): boolean {
  ...
  if (!result) {
    ...
    const hasBudgetWord = new RegExp(
      `\\b(${budgetWordsForRegex})\\b`,
      "i",
    ...
    const hasBudgetPhrase =
      ...
      /\b(\d+)\s*(euro|eur|€)\s*(zur\s+verfügung|verfügbar|habe|hast)\b/i.test(
        text,
      ) ||
      /\b(habe|hast|hat)\s*(ungefähr|etwa|ca\.?|circa)?\s*(\d+)\s*(euro|eur|€)\b/i.test(
        text,
      );
  ...
  if (!result) {
    ...
    const mentionsBoard =
      normalized.includes("board") || normalized.includes("boards");
    const hasBudgetInText =
      /\b(\d+)\s*(euro|eur|€|dollar|\$)\b/i.test(text) ||
      /\b(unter|über|bis|ab|zwischen|von|bis zu|maximal|mindestens|höchstens)\s*\d+/i.test(
        text,
      ) ||
      /\bzwischen\s+(\d+)\s+(und|bis|-)\s*(\d+)\s*(euro|eur|€)\b/i.test(text);
    ...
    if (mentionsBoard && hasBudgetInText && !normalized.includes("snowboard")) {
      ...
      console.log(
        "[EFRO ProductRelated] CLUSTER 1 FIX S4v2 - Board + Budget detected",
        {
          text,
          mentionsBoard,
          hasBudgetInText,
  ...
  if (!result) {
    const normalized = normalize(text);
    const mentionsBindungen =
      normalized.includes("bindungen") ||
  ...
  if (!result) {
    ...
    if (detectedCode && looksLikeProductCode(detectedCode)) {
      ...
      reason = "product_code";
      console.log(
        "[EFRO ProductRelated] CLUSTER 2 FIX K16v1 - Product code detected",
        {
          text,
          detectedCode,
        },
  ...
  if (!result) {
    ...
    if (mentionsBoard) {
      ...
      console.log(
        "[EFRO ProductRelated] CLUSTER 2 FIX PROFI-08v1 - Board detected",
        {
          text,
          mentionsBoard,
        },
  ...
  if (!result) {
    ...
    if (mentionsSmartphone || mentionsMode) {
      result = true;
      reason = mentionsSmartphone
        ? "coreProductKeyword_smartphone"
        : "coreProductKeyword_mode";
      console.log(
        "[EFRO ProductRelated] CLUSTER K FIX - Smartphone/Mode detected",
        { text, mentionsSmartphone, mentionsMode },
      );
    }
  }
  ...
  if (!result) {
    ...
    const mentionsWerkzeug =
      normalized.includes("werkzeug") ||
      normalized.includes("tools") ||
      normalized.includes("tool");

    if (mentionsParfum || mentionsWerkzeug) {
      result = true;
      reason = mentionsParfum
        ? "coreProductKeyword_parfum"
        : "coreProductKeyword_werkzeug";
      console.log(
        "[EFRO ProductRelated] D1v2/H3v2 Fix - Parfüm/Werkzeug detected",
        { text, mentionsParfum, mentionsWerkzeug },
      );
    }
  }
...
function buildAttributeIndex(allProducts: EfroProduct[]): AttributeIndex {
  ...
  function detectSkinType(text: string, productId: string): void {
    ...
    if (/fettige\s+haut|fettiger\s+haut|fettig|oily\s+skin/.test(normalized)) {
...
function parseQueryForAttributes(text: string): ParsedQuery {
  ...
  if (
    /f?r\s+babys|f?r\s+babies|for\s+baby\b|\bbaby\b|\bbabies\b/.test(normalized)
  ) {
  ...
  if (
    /reiniger|cleaner|reinigungs|cleaning|schmutz|verschmutz|verschmutzungen|fleck|flecken|kalk/.test(
      normalized,
    )
  ) {
...
function getClosestCatalogTokens(
  ...
  for (const keyword of catalogKeywords) {
    ...
    const lengthDiff = Math.abs(
      normalizedKeyword.length - normalizedTerm.length,
    );
    const contains =
...
function resolveUnknownTerms(
  ...
  if (!aliasMapUsed && unknownTermsSet.size > 0 && knownSet.size > 0) {
    for (const unknown of unknownTermsSet) {
      const fuzzyMatches = getClosestCatalogTokens(
        unknown,
        Array.from(knownSet),
      );
  ...
  if (
    !aliasMapUsed &&
    fuzzyResolvedSet.size === 0 &&
    unknownTermsSet.size > 0 &&
    knownSet.size > 0
  ) {
    for (const unknown of unknownTermsSet) {
  ...
  console.log("[EFRO UnknownTermsResolved]", {
    ...
    lookupKey:
      uniqUnknown.length > 0 ? normalizeAliasKey(uniqUnknown[0]) : null,
    lookupResult:
      uniqUnknown.length > 0
        ? aliasMap?.[normalizeAliasKey(uniqUnknown[0])]
        : null,
...
async function buildFilterContext(
  ...
  if (userAsksForPerfume && hasPerfumeCategoryInCatalog) {
    ...
    if (perfumeCategory) {
      ...
      if (perfumeProductCount > 0) {
        ...
        console.log(
          "[EFRO Category] Perfume category forced from query (with products)",
          {
  ...
  for (const token of tokens) {
    ...
    const dyn = dynEntries.find(
      (e) => lower.includes(e.term) || e.term.includes(lower),
    );
    if (dyn?.canonicalCategory) {
      dynamicCategoryHints.push(dyn.canonicalCategory);
      console.log(
        "[EFRO DynamicSynonym] Kategorie aus dynamischem Synonym erkannt",
        {
          term: dyn.term,
  ...
  if (
    ...
    console.log(
      "[EFRO IntentFix] Upgraded explore -> quick_buy for 'zeige mir' pattern",
      {
        text,
        words,
        wordCount,
...
function computeBudgetAndPriceRange(
  ...
  if (minPrice !== null || maxPrice !== null) {
    ...
    const isGlobalBudgetOnly = !effectiveCategorySlug && !contextCategory;
    ...
    if (
      ...
      console.log("[EFRO SB] PRICE RANGE NO MATCH", {
        ...
        priceRangeInfo,
        note: "Keine oder nur sehr unpassende Produkte im gew?nschten Preisbereich gefunden.",
      });
    }
  }
...
function resolveUnknownTermsForFilter(
  ...
  const aliasResult = resolveUnknownTerms(
    text,
    catalogKeywords,
    filteredAliasMap,
  );
  ...
  if (aliasResult.resolved.length > 0) {
    ...
    if (aliasResult.aliasMapUsed && aliasResult.resolved.length > 0) {
      ...
      const effectiveWordsSet = new Set<string>([...words, ...aliasResolved]);

      words = Array.from(effectiveWordsSet);
      expandedWords = Array.from(effectiveWordsSet);
    } else {
      // Fallback: Normale Erweiterung
      const updatedWords = Array.from(new Set([...words, ...resolvedSet]));

      const updatedExpandedWords = Array.from(
        new Set([...(expandedWords || []), ...resolvedSet]),
      );
...
function createBudgetFilteredProducts(
  ...
  if (aboveBudget.length > 0) {
    ...
    const sortedAboveBudget = [...aboveBudget].sort(
      (a, b) => (a.price ?? 0) - (b.price ?? 0),
    );

    // Nimm die 3-5 günstigsten (max 5)
    const fallbackProducts = sortedAboveBudget.slice(
      0,
      Math.min(5, sortedAboveBudget.length),
    );
...
function applyProductFilters(
  ...
  if (!effectiveCategorySlug && allProducts.length > 0) {
    const slugs = Array.from(
      new Set(
        allProducts.map((p) => normalize(p.category || "")).filter(Boolean),
      ),
    );
  ...
  if (effectiveCategorySlug) {
    ...
    const filteredByCategory = candidates.filter(
      (p) =>
        normalize(p.category || "") === normalize(effectiveCategorySlug || ""),
    );
    ...
    if (filteredByCategory.length > 0) {
      ...
      if (beforeCategoryFilterCount > 0) {
        debugFlags.push(
          "category_no_match_for_slug:" + String(effectiveCategorySlug),
        );
      }
    ...
    if (!hasCategoryFromQuery) {
      ...
      const hasHairKeywords = WAX_HAIR_KEYWORDS.some((kw) =>
        normalizedText.includes(kw),
      );
      const hasSnowboardKeywords = WAX_SNOWBOARD_KEYWORDS.some((kw) =>
        normalizedText.includes(kw),
      );

      // Pr?fe auf explizite Negationen
      const hasHairNegation =
        /nicht.*haar|kein.*haar|nicht.*frisur|kein.*frisur/i.test(text);
      const hasSnowboardNegation =
        /nicht.*snowboard|kein.*snowboard|nicht.*ski|kein.*ski|nicht.*board|kein.*board/i.test(
          text,
        );
      ...
      const fallbackMatches = candidates.filter((p) => {
        ...
        if (hasWaxKeyword) {
          ...
          const hasWaxInTags =
            ...
            p.tags.some(
              (tag: string) =>
                tag.toLowerCase().includes("wax") ||
                tag.toLowerCase().includes("wachs"),
            );

          if (hasWaxInTitle || hasWaxInTags) {
            // Pr?fe, ob Produkt zu Intent passt
            const isSnowboardWax =
              haystack.includes("snowboard") ||
              haystack.includes("ski") ||
              haystack.includes("belag") ||
              (Array.isArray(p.tags) &&
                p.tags.some((tag: string) =>
                  ["sport", "winter", "accessory", "accessories"].includes(
                    tag.toLowerCase(),
                  ),
                ));
            const isHairWax =
              haystack.includes("hair") ||
      ...
      if (fallbackMatches.length > 0) {
        ...
        console.log(
          "[EFRO SB Fallback] Keine Kategorie erkannt, Fallback-Suche in Titel/Beschreibung/Tags",
          {
            text: text.substring(0, 80),
            ...
            hasWaxKeyword,
            waxProductsFound: hasWaxKeyword
    ...
    if (userMinPrice !== null || userMaxPrice !== null) {
      if (!contextCategory) {
        console.log(
          "[EFRO SB] CATEGORY FILTER SKIPPED (budget-only query, no context)",
          {
            text: text.substring(0, 80),
            userMinPrice,
            userMaxPrice,
        ...
        console.log(
          "[EFRO SB] CATEGORY FILTER APPLIED (budget-only query, using context)",
          {
            text: text.substring(0, 80),
            userMinPrice,
            userMaxPrice,
    ...
    const filteredByAttributes = candidatesAfterAttr.filter((product) => {
      ...
      return activeAttributeFilterEntries.every(([key, values]) => {
        ...
        if (key === "pet") {
          ...
          const speciesMatchesGeneric = (wantsDog || wantsCat) && hasGeneric;
  ...
  if (minPrice !== null || maxPrice !== null) {
    ...
    const budgetResult = createBudgetFilteredProducts(
      candidates,
      minPrice,
      maxPrice,
    );
    ...
    nearestProductTitleAboveBudget =
      budgetResult.nearestProductTitleAboveBudget;
    ...
    console.log("[EFRO SB] AFTER PRICE FILTER", {
      ...
      usingAboveBudgetFallback:
        budgetResult.inBudget.length === 0 &&
        budgetResult.aboveBudget.length > 0,
...
async function filterProducts(
  ...
  return await filterProductsForSellerBrain(
    text,
    intent,
    allProducts,
    contextCategory,
...
function detectProfisellerScenario(
  ...
  if (hasBudget) {
    ...
    // Importiert aus languageRules.de.ts
    const hasOnlyBudgetKeywords =
      BUDGET_KEYWORDS_FOR_SCENARIO.some((kw) => normalized.includes(kw)) &&
      attributeTerms.length === 0 &&
      !BUDGET_ONLY_STOPWORDS.some((stopword) =>
    ...
    if (
      hasOnlyBudgetKeywords ||
      (hasEuroNumberPattern && attributeTerms.length === 0)
    ) {
      return "S4";
    }
  }
  ...
  if (intent === "explore" && !hasBudget && count > 0) {
    const normalized = normalize(text);
    const hasCategoryKeyword = CATEGORY_KEYWORDS_FOR_SCENARIO.some((kw) =>
...
export async function runOrchestrator({
  ...
  if (!Array.isArray(allProducts) || allProducts.length === 0) {
    logWarn("[EFRO SB] Empty product list", {
      ...
      allProductsLength: Array.isArray(allProducts)
        ? allProducts.length
        : "not an array",
    });
  ...
  const previousRecommendedCount = previousRecommended
    ? previousRecommended.length
    : 0;
  ...
  const effectiveExplanationModeForWax =
    explanationMode || (explanationModeBoolean ? "usage" : null);
  ...
  if (effectiveExplanationMode) {
    ...
    const hasCategoryFromQuery =
      CATEGORY_KEYWORDS &&
      Object.values(CATEGORY_KEYWORDS).some((keywords) =>
    ...
    const productsToCheck =
      previousRecommended && previousRecommended.length > 0
        ? previousRecommended
        : recommended;
    ...
    if (productsToCheck.length > 0) {
      if (hasUsableDescription && waxProductWithDescription) {
        ...
        const productTitle =
          waxProductWithDescription.title || "dieses Produkt";
        ...
        if (steps.length > 0) {
          replyText =
            `Hier ist die Anleitung für ${productTitle}:\n\n` +
            steps
        ...
        replyText =
          "F?r dieses Produkt liegt aktuell keine ausf?hrliche Beschreibung im Shop vor. " +
    ...
    console.log(
      "[EFRO SellerBrain] Explanation mode ? Direkt aus Beschreibung",
      {
        text: cleaned,
        explanationMode,
        ...
        hasUsableDescription,
        descriptionLength: waxProductWithDescription
          ? (waxProductWithDescription.description || "").length
          : 0,
        stepsExtracted:
          hasUsableDescription && waxProductWithDescription
            ? extractStepsFromDescription(
                waxProductWithDescription.description || "",
              ).length
            : 0,
      },
    );
    ...
    if (hasUsageButEmpty) {
      console.log(
        "[EFRO SB Explanation Fallback] Usage-Mode ohne Produkte/Beschreibung → FilterProducts",
        {
      ...
      return {
        ...
        debugFlags: hasUsableDescription
          ? undefined
  ...
  const hasBudgetInTextForOffTopic =
    /\b(\d+)\s*(euro|eur|€|dollar|\$)\b/i.test(cleaned) ||
    /\b(unter|über|bis|ab|zwischen|von|bis zu|maximal|mindestens|höchstens)\s*\d+/i.test(
      cleaned,
    ) ||
    /\bzwischen\s+(\d+)\s+(und|bis|-)\s*(\d+)\s*(euro|eur|€)\b/i.test(cleaned);
  const isBudgetWithContext =
    hasContextCategoryForOffTopic && hasBudgetInTextForOffTopic;
  ...
  const hasUnknownTermsWithContext = (() => {
    ...
    const words = normalizedForOffTopic
      .split(/\s+/)
      .filter((w) => w.length >= 3);
    return words.some((w) => {
      const isKnown =
        CORE_PRODUCT_KEYWORDS.some((kw) => normalize(kw).includes(w)) ||
  ...
  const hasElektronikContextForOffTopic =
    context?.activeCategorySlug &&
    normalize(context.activeCategorySlug) === "elektronik";
  const hasSmartphoneAttributesForOffTopic =
    normalizedForOffTopic.includes("schwarz") ||
    normalizedForOffTopic.includes("zoll") ||
    normalizedForOffTopic.includes("display") ||
    normalizedForOffTopic.includes("farbe");
  const isElektronikContextWithAttributes =
    hasElektronikContextForOffTopic && hasSmartphoneAttributesForOffTopic;
  ...
  const isBudgetWithUnknownTerms =
    hasBudgetInTextForOffTopic && hasUnknownTermsInText;
  ...
  const hasBudgetWordForOffTopicCheck =
    /\b(budget|preis|kosten|maximal|mindestens|höchstens|unter|über|bis|ab)\b/i.test(
      cleaned,
    );
  const isBudgetOnlyQueryForOffTopic =
    hasBudgetNumberForOffTopic || hasBudgetWordForOffTopicCheck;
...
// NEW: Treat common “shopping phrasing” as product-related (prevents false off-topic)
const normalizedForOffTopicGate = normalizeText(cleaned);
...
console.log("[EFRO OffTopicGate Debug]", {
  ...
  if (
    ...
    !hasDirectCatalogNameHitForOffTopic &&
    !isBudgetWithContext &&
    !isElektronikContextWithAttributes &&
  ...
  const { userMinPrice, userMaxPrice, hasBudgetWord, isBudgetAmbiguous } =
    budgetAnalysis;

  const hasUserPriceRange = userMinPrice !== null || userMaxPrice !== null;
  ...
  const isPremiumWithMostExpensive =
    nextIntent === "premium" && wantsMostExpensiveForBudget;
  ...
  if (hasUserPriceRange) {
    ...
    if (productsInPriceRange.length === 0) {
      ...
      if (productsAboveBudget.length > 0) {
        ...
        const sortedAboveBudget = [...productsAboveBudget].sort(
          (a, b) => (a.price ?? 0) - (b.price ?? 0),
        );
        ...
        console.log(
          "[EFRO SB] PRICE RANGE NO MATCH detected (with above-budget fallback)",
          {
            text: cleaned.substring(0, 100),
            userMinPrice,
            userMaxPrice,
  ...
  const isBudgetOnly =
    ...
    (() => {
      ...
      const hasProductKeywordFromConfig = PRODUCT_KEYWORDS_FOR_BUDGET_ONLY.some(
  ...
  const wantsOnlyOneProduct =
    /\b(nur|genau|einziges|einzelnes|einzelne|einzelner|ausschließlich|ausschliesslich)\b/i.test(
      cleaned,
    ) && isProductRelated(cleaned);
  ...
  if ((userMinPrice !== null || userMaxPrice !== null) && !priceRangeNoMatch) {
    ...
    if (finalRankedWithBudget.length !== finalRanked.length) {
      console.log(
        "[EFRO Budget-Filter Final] Additional budget filter applied",
        {
          text: cleaned.substring(0, 100),
          userMinPrice,
          userMaxPrice,
  ...
  const hasExactSmartphoneModelName =
    /\b(alpha\s+\d+\s*gb(?:\s+schwarz)?|smartphone\s+alpha\s+\d+\s*gb|das\s+alpha\s+\d+\s*gb\s+schwarz)\b/i.test(
      cleaned,
    );
  if (
    hasExactSmartphoneModelName &&
    recommended.length > 0 &&
    ...
    if (!modelNameInRecommended) {
      ...
      if (effectiveCategorySlug === "elektronik") {
        recommended = [];
        console.log(
          "[EFRO CLUSTER K FIX K6v2] Exakter Smartphone-Modellname gesucht, aber nicht gefunden - Produkte verworfen",
          {
            text: cleaned.substring(0, 100),
            effectiveCategorySlug,
  ...
  if (recommended.length === 0 && nextIntent === "premium") {
    ...
    // F2: Premium-Parf?m
    if (
      normalizedText.includes("parfum") ||
      normalizedText.includes("parf?m") ||
      normalizedText.includes("perfume")
    ) {
    ...
    // K3: Premium-Wasserkocher ab 60 ?
      ...
      normalizedText.includes("wasserkocher") ||
      normalizedText.includes("wasser kocher")
    ) {
      ...
      const kettleProducts = allProducts.filter((p) => {
        ...
        const price = p.price ?? 0;
        return (
          (title.includes("wasserkocher") ||
            category.includes("wasserkocher")) &&
          price >= minPriceForK3
        );
      });

      if (kettleProducts.length > 0) {
        const sortedByPrice = [...kettleProducts].sort(
          (a, b) => (b.price ?? 0) - (a.price ?? 0),
        );
        recommended = sortedByPrice.slice(
          0,
          Math.min(3, kettleProducts.length),
        );
    ...
    if (
      recommended.length > 0 &&
      (!replyText || replyText.includes("helfe dir nur"))
    ) {
      replyText = buildReplyText(
        cleaned,
        nextIntent,
  ...
  const aliasMap = await initializeAliasMap(
    Array.from(catalogKeywordsSetForAlias),
    context?.shopDomain,
    dynamicAliasesForCodeDetect,
  );
  const aliasCheckResult = resolveUnknownTerms(
    cleaned,
    Array.from(catalogKeywordsSetForAlias),
    aliasMap,
  );
  const aliasMatchSuccessful =
    aliasCheckResult.aliasMapUsed && candidateCount > 0;
  ...
  let effectiveCategorySlugForCodeDetect: string | null =
    categoryResultForCodeDetect.effectiveCategorySlug;
  if (
  ...
  const hasBudgetWordForCodeDetect =
    BUDGET_WORD_PATTERNS.some((pattern) =>
      originalForBudget.includes(pattern),
    ) ||
    /\b(budget|preis|maximal|max|h?chstens|hoechstens|nicht mehr als|unter|bis)\b/i.test(
      cleaned,
    );
  ...
  if (detectedCodeTerm) {
    ...
    const productCodeExistsInCatalog = allProducts.some((p) => {
      ...
      if (typeof (p as any).category === "string")
        fields.push((p as any).category);
      if (typeof (p as any).description === "string")
        fields.push((p as any).description);
    ...
    const isLikelyCategoryCodeTerm =
      ...
      (detectedCodeTerm
        .toLowerCase()
        .includes(effectiveCategorySlugForCodeDetect) ||
        effectiveCategorySlugForCodeDetect.includes(
          detectedCodeTerm.toLowerCase(),
        ));
    ...
    if (hasBudgetWordForCodeDetect || isBudgetOnly || isCheapestLikeQuery) {
      ...
      console.log(
        "[EFRO CodeDetect] Budget-Only-Query erkannt, CodeDetect blockiert",
        {
          text: cleaned,
          detectedCodeTerm,
      ...
      unknownProductCodeOnly = false;
      console.log(
        "[EFRO CodeDetect] AliasMatch erfolgreich, CodeDetect blockiert",
        {
          text: cleaned,
          detectedCodeTerm,
          aliasMatchSuccessful,
      ...
      if (isLikelyCategoryCodeTerm) {
        ...
        unknownProductCodeOnly = false;
        console.log(
          "[EFRO CodeDetect] Kategorie-Code erkannt, CodeDetect blockiert",
          {
            text: cleaned,
            detectedCodeTerm,
      ...
      if (isPremiumIntent && isPremiumProdukteTerm) {
        unknownProductCodeOnly = false;
        console.log(
          "[EFRO CodeDetect] Premium-Intent + premium-produkte erkannt, CodeDetect blockiert",
          {
            text: cleaned,
            detectedCodeTerm,
            nextIntent,
      ...
      if (isPremiumIntent && isPremiumVarianteTerm) {
        unknownProductCodeOnly = false;
        console.log(
          "[EFRO CodeDetect] Premium-Intent + premium-variante erkannt, CodeDetect blockiert",
          {
            text: cleaned,
            detectedCodeTerm,
            nextIntent,
      ...
      const hasWasserkocherInText =
        cleaned.toLowerCase().includes("wasserkocher") ||
        cleaned.toLowerCase().includes("kettle") ||
        cleaned.toLowerCase().includes("electric kettle");
      const isPremiumWasserkocherTerm =
        detectedCodeTerm &&
      ...
      if (hasWasserkocherInText && isPremiumWasserkocherTerm) {
        unknownProductCodeOnly = false;
        console.log(
          "[EFRO CodeDetect] Wasserkocher erkannt, CodeDetect blockiert (K3 Fix)",
          {
            text: cleaned,
            detectedCodeTerm,
      ...
      const hasJeansInText =
        cleaned.toLowerCase().includes("jeans") ||
        (cleaned.toLowerCase().includes("slim") &&
          cleaned.toLowerCase().includes("fit"));
      const isJeansTerm =
        detectedCodeTerm &&
      ...
      if (
        hasJeansInText &&
        isJeansTerm &&
        effectiveCategorySlugForCodeDetect === "mode"
      ) {
        unknownProductCodeOnly = false;
        console.log(
          "[EFRO CodeDetect] Jeans-Term erkannt, CodeDetect blockiert (K10v1/K11v1 Fix)",
          {
            text: cleaned,
            detectedCodeTerm,
            hasJeansInText,
    ...
    if (unknownProductCodeOnly) {
      ...
      const hasProductsInContextCategory =
        hasContextCategory &&
        recommended.some(
      ...
      const hasProductsInEffectiveCategory = effectiveCategorySlug
        ? (() => {
            const normalizedEffectiveCategory = normalize(
              effectiveCategorySlug,
            );
            return recommended.some(
              (p) =>
                normalize(p.category || "") === normalizedEffectiveCategory,
            );
          })()
        : false;
      ...
      const hasExactSmartphoneModelNameForCodeDetect =
        /\b(alpha\s+\d+\s*gb(?:\s+schwarz)?|smartphone\s+alpha\s+\d+\s*gb|das\s+alpha\s+\d+\s*gb\s+schwarz)\b/i.test(
          cleaned,
        );
      const shouldAllowSmartphoneExactMatch =
        hasExactSmartphoneModelNameForCodeDetect &&
        effectiveCategorySlug === "elektronik";
      ...
      const hasJeansQueryForCodeDetect =
        normalizedForJeansCodeDetect.includes("jeans");
      const shouldAllowJeansFallback =
        hasJeansQueryForCodeDetect && effectiveCategorySlug === "mode";
      ...
      if (
        (!aliasMatchSuccessful || candidateCountAfterAlias === 0) &&
        !hasProductsInContextCategory &&
        !hasProductsInEffectiveCategory &&
        !shouldAllowSmartphoneExactMatch &&
        !shouldAllowJeansFallback
      ) {
        recommended = [];
        console.log(
          "[EFRO CodeDetect] Produkte verworfen (unknownProductCodeOnly = true)",
          {
            text: cleaned,
            detectedCodeTerm,
            aliasMatchSuccessful,
        ...
        if (hasProductsInContextCategory || hasProductsInEffectiveCategory) {
          console.log(
            "[EFRO CodeDetect] Produkte NICHT verworfen (Kategorie mit Produkten)",
            {
              text: cleaned,
              detectedCodeTerm,
          ...
          console.log(
            "[EFRO CodeDetect] Produkte NICHT verworfen (AliasMatch erfolgreich)",
            {
              text: cleaned,
              detectedCodeTerm,
              aliasMatchSuccessful,
  ...
  const resolvedTermsSet = new Set(
    (aliasCheckResult?.resolved || []).map((t) =>
      normalizeAliasKey(t.toLowerCase()),
    ),
  );
  const knownKeywordsNormalized = new Set(
    Array.from(catalogKeywordsSetForAlias || []).map((kw) =>
      normalizeAliasKey(kw.toLowerCase()),
    ),
  );
  ...
  const filteredUnknownTerms = (aiUnknownTerms || [])
    ...
    .filter((t) => {
      ...
      return (
        !resolvedTermsSet.has(normalized) &&
        !knownKeywordsNormalized.has(normalized)
      );
    });
  ...
  if (isBudgetSmalltalkCheck(normalizedTextForSmalltalk)) {
    ...
    console.log(
      "[EFRO SB AI-Trigger] J3v2 Fix - Budget-Smalltalk erkannt, KEIN AI-Trigger",
      {
        text: cleaned.substring(0, 100),
  ...
  if (priceRangeInfo) {
    ...
    const effectiveCategorySlugForDecide =
      effectiveCategorySlugForCodeDetect || context?.activeCategorySlug || null;
    const productsInCategoryForDecide = effectiveCategorySlugForDecide
      ? allProducts.filter(
          (p) =>
            normalize(p.category || "") ===
            normalize(effectiveCategorySlugForDecide),
        )
      : allProducts;
    ...
    categoryMinPriceForDecide =
      pricesInCategoryForDecide.length > 0
        ? pricesInCategoryForDecide[0]
        : null;
    categoryMaxPriceForDecide =
      pricesInCategoryForDecide.length > 0
        ? pricesInCategoryForDecide[pricesInCategoryForDecide.length - 1]
        : null;
  }
  ...
  if (
    detectedCodeTerm &&
    (priceRangeInfo || hasUserPriceRange || hasBudgetWord)
  ) {
    const codeLc = detectedCodeTerm.toLowerCase();
    const productCodeExistsInCatalog = allProducts.some((p) => {
      const fields: string[] = [];
    ...
    if (
      !productCodeExistsInCatalog &&
      !unknownTermsForDecide.includes(codeLc)
    ) {
  ...
  const shouldCallDecideAiTrigger =
    (priceRangeInfo || hasUserPriceRange || hasBudgetWord) &&
  ...
  if (shouldCallDecideAiTrigger) {
    ...
    if (budgetAiTrigger) {
      ...
      budgetAiTrigger === undefined &&
      (priceRangeInfo?.userMaxPrice ?? userMaxPrice) !== null &&
      categoryMaxPriceForDecide !== null &&
      categoryMinPriceForDecide !== null &&
      (priceRangeInfo?.userMaxPrice ?? userMaxPrice ?? 0) >=
        categoryMaxPriceForDecide * 1.5
    ) {
      // J1-Fall: Sehr hohes Budget ? KEIN AI-Trigger
      // Stelle sicher, dass kein AI-Trigger f?r Budget-F?lle gesetzt wird
      console.log(
        "[EFRO SB decideAiTrigger] Sehr hohes Budget erkannt, KEIN AI-Trigger (J1)",
        {
          text: cleaned.substring(0, 100),
          userMaxPrice: priceRangeInfo?.userMaxPrice ?? userMaxPrice,
          categoryMaxPrice: categoryMaxPriceForDecide,
  ...
  if (possibleProductCodes.length > 0) {
    for (const code of possibleProductCodes) {
      const codeExists = allProducts.some((p) => {
        ...
        if (typeof (p as any).sku === "string") fields.push((p as any).sku);
        if (typeof (p as any).title === "string") fields.push((p as any).title);
        if (typeof (p as any).handle === "string")
          fields.push((p as any).handle);
  ...
  hasFressnapf = filteredUnknownTerms.some(
    (t) =>
      normalizeAliasKey(t) === "fressnapf" ||
      t.toLowerCase().includes("fressnapf"),
  );
  ...
  const isPureBudgetQuery =
    hasBudgetWord && hasNoNumber && !isProductRelated(cleaned);
  ...
  if (
    ...
    console.log(
      "[EFRO SB AI-Trigger] Skipped for budget-only query (no unknown codes/brands)",
      {
        text: cleaned.substring(0, 100),
        finalCount,
        hasBudgetWord,
    ...
    console.log(
      "[EFRO SB AI-Trigger] Skipped for pure budget-only query (clarify in replyText)",
      {
        text: cleaned.substring(0, 100),
        finalCount,
        hasBudgetWord,
    ...
    if (isGlobalBudgetOnly && !hasBudgetTriggerFromDecide) {
      ...
      if (isExplanationIntent && hasRecommendations) {
        ...
        reason = "";
        console.log(
          "[EFRO SB AI-Trigger] Skipped for explanation with product match",
          {
            text: cleaned.substring(0, 100),
            explanationMode: effectiveExplanationMode,
            finalCount,
        ...
        if (
          ...
          if (hasUnknownKeywords) {
            ...
            reason = "";
            console.log(
              "[EFRO SB AI-Trigger] Skipped for no results without unknown keywords",
              {
                text: cleaned.substring(0, 100),
                finalCount,
                filteredUnknownCount,
          ...
          if (
            !skipForHighBudgetOnly &&
            !(aiTrigger && aiTrigger.reason === "budget_smalltalk_no_ai")
          ) {
            needsAiHelp = true;
        ...
        if (!needsAiHelp && finalCount > 0) {
          ...
          if (hasUnknownTerms && hasBudgetInfo && !skipForHighBudgetOnly) {
            ...
            console.log(
              "[EFRO SB AI-Trigger] Set for unknown_term_with_budget",
              {
                text: cleaned.substring(0, 100),
                filteredUnknownCount,
                unknownProductCodeOnly,
                unknownProductCodesCount:
                  unknownProductCodesForTrigger?.length ?? 0,
                hasBudgetWord,
                userMinPrice,
                userMaxPrice,
        ...
        if (filteredUnknownTerms.length > 0) {
          ...
          const dynamicSynonymTerms = new Set(
        ...
        if (
          ...
          if (unknownProductCodeOnly && detectedCodeTerm) {
            ...
            const codeFromTerms = aiUnknownTerms.find((t) =>
              looksLikeProductCode(t),
            );
            if (codeFromTerms) {
          ...
          if (hasFressnapf) {
            const fressnapfTerms = filteredUnknownTerms.filter(
              (t) =>
                normalizeAliasKey(t) === "fressnapf" ||
                t.toLowerCase().includes("fressnapf"),
            );
          ...
          if (
            ...
            (reason === "unknown_term_expansion" ||
              reason === "low_confidence_unknown_terms" ||
              reason === "synonym_lookup" ||
              reason === "unknown_term_with_budget")
          ) {
            aiTrigger.termExplainRequests = filteredUnknownTerms.map(
              (term) => ({
                term,
        ...
        // Pr?fe, ob decideAiTrigger bereits einen Budget-Trigger zur?ckgegeben hat
        const hasBudgetTriggerFromDecide =
          aiTrigger?.reason === "budget_very_low";
  ...
  if (!aiTrigger && hasBudget) {
    ...
    if (isBudgetOnly && hasNumericBudget && maxBudget !== undefined) {
      if (maxBudget < HIGH_BUDGET_THRESHOLD) {
        ...
        console.log(
          "[EFRO SB AI-Trigger] Skipped for very_high_budget_with_results",
          {
            text: cleaned.substring(0, 100),
            userMinPrice,
            userMaxPrice,
  ...
  if (
    ...
    aiTrigger = {
      needsAiHelp: true,
      reason: reasonKey,
      unknownTerms: [...(aiTrigger?.unknownTerms || []), ...codeTerms],
      codeTerm:
        aiTrigger?.codeTerm || detectedCodeTerm || (codeTerms[0] ?? undefined),
      unknownProductCodes:
        codeTerms.length > 0 ? codeTerms : aiTrigger?.unknownProductCodes,
    };
  ...
  if (!effectiveCategorySlug) {
    ...
    console.log(
      "[EFRO SB Context] Keeping effectiveCategorySlug from Category-Modul (F7 Fix)",
      {
        effectiveCategorySlug,
        recommendedCount: recommended.length,
        firstProductCategory:
          recommended.length > 0 ? recommended[0].category : null,
  ...
  const isBudgetOnlyQuery = (() => {
    ...
    const parsed = parseQueryForAttributes(cleaned);
    // Importiert aus languageRules.de.ts
    const hasProductKeyword = PRODUCT_KEYWORDS_FOR_BUDGET_ONLY.some((kw) =>
      normalized.includes(kw),
    );
    return (
      hasBudgetKeyword &&
      parsed.attributeTerms.length === 0 &&
      !hasProductKeyword
    );
  })();
...
// - Zusätzlich nur den AI-Trigger nutzen, um den unbekannten Begriff zu klären
  if (
    ...
    if (finalCount === 0) {
      ...
      recommended = [];
      console.log(
        "[EFRO SB] Produkte verworfen (unknown_product_code_only, kein AliasMatch, finalCount=0)",
        {
          text: cleaned.substring(0, 100),
          reason: aiTrigger.reason,
          aliasMatchSuccessful,
      ...
      console.log(
        "[EFRO SB] Produkte NICHT verworfen (unknown_product_code_only, aber finalCount > 0)",
        {
          text: cleaned.substring(0, 100),
          reason: aiTrigger.reason,
          aliasMatchSuccessful,
    ...
    console.log(
      "[EFRO SB] Produkte NICHT verworfen (unknown_product_code_only, aber AliasMatch erfolgreich)",
      {
        text: cleaned.substring(0, 100),
        reason: aiTrigger.reason,
        aliasMatchSuccessful,
  ...
  const categoryHintsFromText = collectMatches(
    normalize(cleaned).toLowerCase(),
    CATEGORY_KEYWORDS,
  );
  if (categoryHintsFromText.length > 0) {
    ...
    const existsInCatalog = allCategories.some(
      (cat) => normalize(cat) === normalizedHint,
    );
    if (!existsInCatalog) {
      missingCategoryHint = firstHint;
      console.log(
        "[EFRO SB] Missing category hint detected in runSellerBrain",
        {
          text: cleaned.substring(0, 100),
          missingCategoryHint,
  ...
  if (priceRangeNoMatch) {
    ...
    if (
      recommended.length === 0 &&
      priceRangeInfo?.nearestPriceAboveBudget === null
    ) {
  ...
  if (
    !finalReplyText ||
    typeof finalReplyText !== "string" ||
    finalReplyText.trim().length === 0
  ) {
  ...
  // WICHTIG: Nicht ausführen, wenn priceRangeNoMatch === true (dann sollen keine Produkte empfohlen werden)
  const hasSmartphoneModelName =
    /\b(smartphone\s+alpha|alpha\s+smartphone|phone\s+alpha|alpha\s+phone|alpha\s+\d+gb|alpha\s+\d+\s*gb|das\s+alpha\s+\d+\s*gb|alpha\s+\d+\s*gb\s+schwarz)\b/i.test(
      cleaned,
    );
  const recommendedProductsDontMatch =
    recommended.length > 0 &&
    hasSmartphoneModelName &&
    !recommended.some((p) => {
      const productTitle = normalize(p.title || "");
      return (
        productTitle.includes("alpha") || productTitle.includes("smartphone")
      );
    });

  if (
    (recommended.length === 0 || recommendedProductsDontMatch) &&
    !priceRangeNoMatch
  ) {
    const normalizedText = normalize(cleaned);
    ...
    const userAsksForSmartphone = smartphoneKeywords.some((kw) =>
      normalizedText.includes(kw),
    );
    const hasAlphaModelName =
      /\b(alpha\s+\d+\s*gb|alpha\s+\d+\s*gb\s+schwarz|das\s+alpha\s+\d+\s*gb)\b/i.test(
        cleaned,
      );
    const targetCategoryForSmartphone =
      userAsksForSmartphone || hasAlphaModelName ? "elektronik" : null;
    ...
    if (productNameFragments.length > 0) {
      ...
      if (exactTitleMatches.length > 0) {
        ...
        recommended = exactTitleMatches.slice(
          0,
          Math.min(2, exactTitleMatches.length),
        );
        console.log(
          "[EFRO AI Smartphone Exact Match] K6 - Exakter Titel-Match (alle Fragmente)",
          {
            text: cleaned.substring(0, 100),
        ...
        const partialMatches = allProducts.filter((p) => {
          ...
          return productNameFragments.some((fragment) =>
            productTitle.includes(fragment),
          );
        });
        ...
        if (
          ...
          const { maxPrice: userMaxPriceForCheck } =
            extractUserPriceRange(cleaned);
          const hasExplicitMaxBudget =
            userMaxPriceForCheck !== null &&
            typeof userMaxPriceForCheck === "number" &&
            !Number.isNaN(userMaxPriceForCheck);

          if (
            hasExplicitMaxBudget &&
            priceRangeNoMatch &&
            userMaxPriceForCheck !== null
          ) {
            ...
            if (filteredMatches.length > 0) {
              ...
              console.log(
                "[EFRO D8 Fix] AI Smartphone Exact Match skipped - products above budget",
                {
                  text: cleaned.substring(0, 100),
          ...
          if (recommended.length > 0) {
            console.log(
              "[EFRO AI Smartphone Exact Match] K6 - Partieller Titel-Match",
              {
                text: cleaned.substring(0, 100),
                productNameFragments,
                recommendedCount: recommended.length,
          ...
          if (realSmartphones.length > 0) {
            ...
            recommended = realSmartphones.slice(
              0,
              Math.min(2, realSmartphones.length),
            );
            ...
            console.log(
              "[EFRO AI Smartphone Fake Model] K16 - Fake-Modellname nicht gefunden, zeige echte Smartphones",
              {
                text: cleaned.substring(0, 100),
                productNameFragments,
                recommendedCount: recommended.length,
  ...
  if (
    ...
    (priceRangeInfo.userMinPrice !== null ||
      priceRangeInfo.userMaxPrice !== null)
  ) {
    const effectiveCategoryForFallback =
      effectiveCategorySlug || context?.activeCategorySlug || null;
  ...
  if (
    ...
    const petFallback = allProducts.filter((p) => {
      ...
      const isPetCategory = cat === "haustier" || cat === "tierbedarf";
  ...
  const wantsMostExpensiveInCategory =
    !!effectiveCategorySlug &&
    (wantsMostExpensiveLegacy || mostExpensiveSignal);
  ...
  // SCHRITT 4 FIX (angepasst): Bei ASK_CLARIFICATION + NO_PRODUCTS_FOUND
  // nur dann Produkte leeren, wenn sowieso keine da sind.
  // Sonst bleiben die Produkte drin (sonst curated1000: count 0 < minCount 1).
  if (
    salesPolicyOutput.primaryAction === "ASK_CLARIFICATION" &&
    salesPolicyOutput.notes?.includes("NO_PRODUCTS_FOUND")
  ) {
    const originalCount = recommended.length;

    if (originalCount === 0) {
      recommended = [];
      console.log(
        "[EFRO SB ASK_CLARIFICATION] Produkte entfernt (NO_PRODUCTS_FOUND)",
        {
          text: cleaned.substring(0, 100),
          originalCount,
          finalCount: 0,
      ...
      console.log(
        "[EFRO SB ASK_CLARIFICATION] NO_PRODUCTS_FOUND ignored (kept products)",
        {
          text: cleaned.substring(0, 100),
          originalCount,
          finalCount: recommended.length,
