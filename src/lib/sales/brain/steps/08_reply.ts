...
export function buildReplyText(
  ...
  if (priceRangeNoMatch && priceRangeInfo) {
    ...
    let requestedRange = "deinem gew�nschten Preisbereich";
    if (userMinPrice !== null && userMaxPrice === null) requestedRange = `�ber ${userMinPrice} �`;
  ...
  if (missingCategoryHint) {
    ...
    if (effectiveReplyMode === "operator") {
      clarifyText +=
        `\n\nHinweis f�r den Shop-Betreiber: In deinem Katalog gibt es aktuell keine ${categoryLabel} � nur ${alternativeCategory} und ggf. Zubeh�r. ` +
  ...
  if (aiTrigger?.reason === "unknown_product_code_only") {
    ...
    return (
      ...
      `Sag mir bitte, was f�r ein Produkt du suchst � zum Beispiel eine Kategorie, eine Marke oder ein Einsatzgebiet. ` +
  ...
  if (aiTrigger?.reason === "ambiguous_budget") {
    return (
      ...
      `� F�r welche Art von Produkt suchst du etwas?\n` +
