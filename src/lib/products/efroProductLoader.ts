...
// EFRO Budget-Fix 2025-11-30: Produkt-Loader fÃ¼r Test-Scripts und App

import type { EfroProduct } from "./mockCatalog";
import { mockCatalog } from "./mockCatalog";
import { efroAttributeTestProducts } from "../catalog/efro-attribute-test-products";

export type { EfroProduct } from "./mockCatalog";
...
// Helper: PrÃ¼ft, ob Mock-Katalog verwendet werden soll
...
/**
 * LÃ¤dt Produkte fÃ¼r einen Shop-Domain
 * 
 * Versucht echte Produkte aus Shopify zu laden (Ã¼ber /api/shopify-products).
 * Bei Fehler oder wenn keine Produkte gefunden werden, fÃ¤llt auf mockCatalog zurÃ¼ck.
 * 
...
/**
 * LÃ¤dt Produkte fÃ¼r local-dev Shop-Domain
 * 
 * EFRO Budget-Fix 2025-11-30: Diese Funktion repliziert die Logik aus page.tsx
 * fÃ¼r Test-Scripts, die ohne Next.js-Server laufen.
 * 
...
export async function loadEfroProductsForLocalDev(): Promise<EfroProduct[]> {
  // FÃ¼r Test-Scripts: Versuche zuerst Shopify-API, dann Fallback auf mockCatalog
  try {
    ...
    // Versuche Shopify-API (nur wenn in Next.js-Umgebung verfÃ¼gbar)
    const url = `https://${SHOP_DOMAIN}/admin/api/2024-01/products.json?limit=50`;
